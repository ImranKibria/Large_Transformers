...
model = Wav2Vec2ForSequenceClassification.from_pretrained(model_checkpoint).to(device)

hidden_size = model.classifier.in_features
num_output_classes = 1
model.classifier = torch.nn.Linear(hidden_size, num_output_classes)

model.load_state_dict(torch.load("wav2vec2-base-960h-finetuned/checkpoint-519/pytorch_model.bin", map_location=torch.device(device)))
print(model)
