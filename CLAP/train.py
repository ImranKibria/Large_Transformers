'Adapting CLAP audio model from Hugging Face for Speech Quality Assessment'
...
audio_dataset = combined_dataset.cast_column("filepath_deg", Audio(sampling_rate=48000))
print(f'\naudio dataset:\n {audio_dataset}')

###
# compute features (compatible clap input) from raw audio.
###
...
###
# load pretrained clap audio model & add MLP to predict mos
###

# model = ClapAudioModel.from_pretrained(model_checkpoint) # load this model to omit projection head
model = ClapAudioModelWithProjection.from_pretrained(model_checkpoint) # load this model to retain projection head
