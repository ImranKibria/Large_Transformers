'Adapting wavLM from Hugging Face for Speech Quality Assessment'
...
batch_size = 16
model_checkpoint = "patrickvonplaten/wavlm-libri-clean-100h-base-plus"
...
###
# load pretrained wavLM with 1-unit head to predict mos
###

model = WavLMForSequenceClassification.from_pretrained(model_checkpoint).to(device)

hidden_size = model.classifier.in_features
num_output_classes = 1
model.classifier = torch.nn.Linear(hidden_size, num_output_classes)

print(f'wavLM model with MOS prediction head:\n {model}')
