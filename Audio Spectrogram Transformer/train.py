'Adapting AST from Hugging Face for Speech Quality Assessment'
...
###
# compute log-mel-spectrum (compatible AST input) from raw audio.
###
...
print(f'\nlog-mel-spectrum dataset:\n {encoded_dataset}')

###
# load pretrained AST with 1-unit head to predict mos
###

model = ASTForAudioClassification.from_pretrained(model_checkpoint).to(device)

hidden_size = model.classifier.dense.in_features
num_output_classes = 1
model.classifier = torch.nn.Linear(hidden_size, num_output_classes)

print(f'\nAST model with MOS prediction head:\n {model}')
...
print(trainer.evaluate(NISQA_VAL_SIM))
...
print(trainer.evaluate(NISQA_VAL_LIVE))
...
print(trainer.evaluate(NISQA_TEST_FOR))
...
print(trainer.evaluate(NISQA_TEST_P501))
...
print(trainer.evaluate(NISQA_TEST_LIVETALK))